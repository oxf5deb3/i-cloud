...
namespace VMS.DAL
{
    public class SqlServerDBHelper: IDbHelper, IDisposable
    {
        ...
        private static string connectionStr = PubConstant.ConnectionString;
        ...
        protected void AddInParameter(DbCommand cmd, SqlParam[] _params)
        {
            if (_params != null)
            {
                for (int i = 0; i < _params.Length; i++)
                {
                    ...
                    if (_param.FieldValue is DateTime)
                    {
                        type = DbType.DateTime;
                    }
                    this.GetDatabase().AddInParameter(cmd, _param.FieldName.Replace(":", "@"), type, _param.FieldValue);
                }
            }
        }
        ...
        public object GetObjectValue(StringBuilder sql, SqlParam[] param)
        {
            ...
            {
                Log4NetHelper.Error(string.Concat(new string[]
				{
					"-----------根据SQL返回影响行数-----------\r\n",
					sql.ToString(),
					"\r\n",
            ...
            try
            {
                ...
                using (DbConnection conn = this.db.CreateConnection())
                {
                    ...
                    catch (Exception e)
                    {
                        trans.Rollback();
                        num = -1;
                        Log4NetHelper.Error(string.Concat(new object[]
						{
							"-----------根据SQL执行,回滚事物-----------\r\n",
							sql.ToString(),
							"\r\n",
        ...
        public int ExecuteSqlTran(string SQLString, params SqlParameter[] cmdParms)
        {
            using (SqlConnection connection = new SqlConnection(connectionStr))
            {
                connection.Open();
        ...
        public DataSet GetDataSetBySQL(StringBuilder sql, SqlParam[] param)
        {
            ...
            {
                Log4NetHelper.Error("-----------获取数据集DataSet-----------\n" + sql.ToString() + "\n" + e.Message);
                result = null;
            }
            return result;
        }
        ...
        public int ExecuteByProcReturn(string procName, Hashtable ht, ref Hashtable rs)
        {
            ...
            {
                Log4NetHelper.Error("-----------执行存储过程返回指定消息-----------\n" + procName + "\n" + exception.Message);
            }
            result = num;
            return result;
        }
